...
│   ├── robots.txt                            → public/ (served at canonical URLs)
...
### 7. (Optional) Design the site in the Design Studio

If you want a per-client visual treatment beyond the default theme, see [Designing the visual look](#designing-the-visual-look) below.
...
├── posts/*.md                ← optional: blog posts served at /insights/<slug>
...
├── og-images/                ← (legacy, optional) per-page PNGs; auto-OG via /api/og now
├── resources/                ← optional: PDFs for the resource-list lead-magnet block
├── content-assets/           ← logo, team photos, hero images
src/app/sitemap.ts            ← dynamic sitemap generator (sole source; pages + posts)
...
Beyond the page assembly system, the template ships several surfaces a client gets for free:

- **Blog at `/insights`** — `content/posts/*.md` populates a date-sorted index + per-post pages with BlogPosting JSON-LD; RSS 2.0 at `/feed.xml`. Empty-state UX until posts exist.
- **Auto OG share cards** at `/api/og` (home) and `/api/og/<slug>` (per page) — 1200×630 PNG rendered from brand colors + page title. No per-page PNG required. (You can still drop a static `opengraph-image.png` next to a route to override.)
- **Auto favicon + Apple touch icon** at `/icon` and `/apple-icon` — initials on brand-primary, generated per request.
- **Booking block** (`<!-- block: booking -->`) — embeds Calendly (or any iframe-compatible scheduler) once `siteConfig.booking` is set.
- **Resource-list block** (`<!-- block: resource-list -->`) — downloadable PDFs from `public/resources/` rendered as cards with an inline newsletter signup CTA below.
- **GA4 `generate_lead` event** fired on every successful form submission (distinguishes `form_submit` vs `mailto_fallback`).
- **Branded 404** with the firm's primary nav as recovery links.
- **Dev-only `/showcase`** — every block in the registry rendered with realistic sample content. Useful for visual reviews before content lands.

**Sitemaps:**
- `/sitemap.xml` is generated dynamically by `src/app/sitemap.ts` from the actual pages in `content/pages/` **plus** every post in `content/posts/` (and the `/resources` index once a post exists). It always reflects current content — add or remove a page or post and the sitemap follows on the next build.
- The deliverable does **not** ship a static `public/sitemap.xml`. A static file in `public/` is served by Next before the route, so it would shadow the generator and freeze the sitemap at packaging time — missing any post drafted later. Don't add one back.
...
The consent banner is targetable like the navbar + footer chrome (`data-component="cookie-consent"` outer element, with `data-slot="message|accept|decline"` on the named child elements). Brand-specific overrides go in `content/design-overrides.css` the same way as any other chrome element — see the "Designing the visual look" section below.

## Designing the visual look

The 22 block components define the *shape* of every page. Their actual visual treatment — colors, gradients, shadows, custom corner radii, spacing nuance — is fed in via three files:

- **`content/design.json`** — design tokens (palette, fonts, radius scale) plus the optional `style` axes (see `src/lib/theme/style-axes.ts`). Drives the generated `theme.css` and the `<html data-c5-*>` presets.
- **`content/design-overrides.css`** — block-specific CSS. Loaded after `theme.css` and `style-axes.css`, so any rule here wins.
- **`src/app/fonts.generated.ts`** — generated by `npm run generate-fonts` from `design.json` typography.

Design work is done in the **Revaltus Design Studio** (in the onboarding platform), which reads the live site plus `/design-specimen` and writes these files back. The old `export-brief` / Claude.ai paste handoff is retired. After editing `design.json` by hand, re-run the generators:
...
The full list of available `data-block` values matches the 22 block IDs documented in `raw-docs/phaseII/component-library-spec.md` (in the onboarding repo).

### Versioning

`content/design-overrides.css` is committed to the repo — each client clone carries its own design decisions in git history. The placeholder ships empty; the Design Studio (or you by hand) fills it. Re-running `npm run unpack` does NOT overwrite this file (the Phase I deliverable doesn't include a `design-overrides.css`), so iteration on the design is safe.
